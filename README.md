...
영문 표시가 필요한 항목은 `text_en` 필드를 함께 적습니다(없으면 한국어 그대로 표시).

### 영어 버전 (한/영 전환)
- 상단 메뉴의 **EN / 한국어** 버튼 또는 `?lang=en` 링크로 전환됩니다. 선택은 브라우저에 기억됩니다.
- 번역은 `assets/i18n.js` 한 파일입니다. HTML은 한국어 원문 그대로 두고, **한국어 문구를 키로** 영어를 찾아 바꿉니다.
  → 한국어 문구를 고치면 `i18n.js`의 키도 같이 고쳐야 영어가 나옵니다(안 고치면 그 문구만 한국어로 보임).
- 새 문구를 추가했을 때 빠진 번역 확인: 영어 모드에서 브라우저 콘솔에 `I18N.missing()` 입력 → 번역 안 된 한국어 목록(회원 이름 등 데이터는 원래 나옵니다).
- 영어로 안 바뀌는 것: 주소록 데이터(이름·소속), 운영 시트에서 오는 문구, 구글폼·이벤터스 화면, 회지 PDF.
